        ...
        None => {
            ...
            if obstacles.is_empty() {
                ...
                auto_radius(&work, gap, options, &obstacles, r_hi)
                    .unwrap_or(0.0)
                    .max(OBSTACLE_RADIUS_FLOOR * r_free)
            }
        }
    };
...
/// How much of the obstacle-blind radius the obstacle-aware bisection is
...
const OBSTACLE_RADIUS_FLOOR: f64 = 0.5;
...
/// point is inside it.
fn dist_to_rect(px: f64, py: f64, rect: &Rectangle) -> f64 {
...
fn clear_of_obstacles(px: f64, py: f64, obstacles: &[Rectangle], clearance: f64) -> bool {
    obstacles
        .iter()
...
/// than an error.
fn sanitize_obstacles(obstacles: &[Rectangle]) -> Vec<Rectangle> {
    obstacles
        .iter()
        .filter(|rect| {
...
fn obstacles_near(piece: &RegionPiece, obstacles: &[Rectangle], clearance: f64) -> Vec<Rectangle> {
    if obstacles.is_empty() {
        return Vec::new();
    }
...
mod tests {
    ...
    fn boxed_region(x0: f64, y0: f64, x1: f64, y1: f64) -> RegionPolygons {
        let mut map = HashMap::new();
        map.insert(
            Combination::new(&["A"]),
    ...
    fn auto_radius_shrinks_but_is_floored_by_obstacles() {
        ...
        assert!(
            result.radius >= OBSTACLE_RADIUS_FLOOR * r_free - 1e-12,
            "radius {} fell below the floor of {}",
            result.radius,
            OBSTACLE_RADIUS_FLOOR * r_free
        );
