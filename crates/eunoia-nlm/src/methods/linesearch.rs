...
pub fn lnsrch(params: &LnsrchParams) -> LnsrchResult {
    const ALPHA: f64 = 1e-4; // Armijo parameter (nlm uses Goldstein-Armijo)
    const LAMBDA_MIN_FACTOR: f64 = 0.1; // min step fraction
    const LAMBDA_MAX_FACTOR: f64 = 0.5; // max step fraction
    const SMALL_DENOM: f64 = 1e-12;
    const MAX_BACKTRACK: usize = 50; // match nlm's higher retry allowance
    let n = params.x.len();
