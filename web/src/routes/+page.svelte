...
<div class="text-ink">
  <main class="max-w-5xl mx-auto px-6 py-16 space-y-16">
      ...
      >
        ...
        <div>
          ...
          <p class="text-lg text-muted mb-4">
            ...
            <a href={eulerrRepoUrl} class="text-accent hover:underline"
              >eulerr</a
            >, npm package
            <a href={npmUrl} class="text-accent hover:underline"
              >@jolars/eunoia</a
            >, Julia package
            <a href={juliaRepoUrl} class="text-accent hover:underline"
              >Eunoia.jl</a
            >, and Python package
            <a href={pypiUrl} class="text-accent hover:underline">eunoia</a>.
          </p>
          <div class="flex flex-wrap gap-3 pt-2">
            ...
            <a
              href={repoUrl}
              class="px-5 py-2.5 rounded border border-line bg-surface text-sm font-medium hover:bg-inset inline-flex items-center gap-2"
    ...
    <section class="space-y-4">
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div class="bg-surface border border-line rounded p-4 space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-medium inline-flex items-center gap-2"
              ><IconRust class="w-4 h-4" /> Rust</span
            >
            <a href={cratesUrl} class="text-xs text-accent hover:underline"
              >crates.io</a
            >
          </div>
          <pre
            class="bg-inset border border-line rounded p-2 font-mono text-xs overflow-x-auto">cargo add eunoia</pre>
          <p class="text-xs text-muted">
            Docs at
            <a href={docsRsUrl} class="text-accent hover:underline"
              >docs.rs/eunoia</a
            >.
          </p>
        </div>
        <div class="bg-surface border border-line rounded p-4 space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-medium inline-flex items-center gap-2"
              ><IconNpm class="w-4 h-4" /> JavaScript / TypeScript</span
            >
            <a href={npmUrl} class="text-xs text-accent hover:underline">npm</a>
          </div>
          <pre
            class="bg-inset border border-line rounded p-2 font-mono text-xs overflow-x-auto">npm install @jolars/eunoia</pre>
          <p class="text-xs text-muted">
            WASM bindings; powers
            <a href="/app/" class="text-accent hover:underline">the web app</a> and
            all the interactive demos on this site.
          </p>
        </div>
        <div class="bg-surface border border-line rounded p-4 space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-medium inline-flex items-center gap-2"
              ><IconR class="w-4 h-4" /> R</span
            >
            <a href={eulerrUrl} class="text-xs text-accent hover:underline"
              >CRAN</a
            >
          </div>
          <pre
            class="bg-inset border border-line rounded p-2 font-mono text-xs overflow-x-auto">install.packages("eulerr")</pre>
          <p class="text-xs text-muted">
            R users can use
            <a href={eulerrRepoUrl} class="text-accent hover:underline"
              >eulerr</a
            >, which is now backed by the same Rust core as Eunoia; docs at
            <a href={eulerrDocsUrl} class="text-accent hover:underline"
              >jolars.github.io/eulerr</a
            >.
          </p>
        </div>
        <div class="bg-surface border border-line rounded p-4 space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-medium inline-flex items-center gap-2"
              ><IconPython class="w-4 h-4" /> Python</span
            >
            <a href={pypiUrl} class="text-xs text-accent hover:underline"
              >PyPI</a
            >
          </div>
          <pre
            class="bg-inset border border-line rounded p-2 font-mono text-xs overflow-x-auto">pip install eunoia</pre>
          <p class="text-xs text-muted">
            Bindings to the Rust core with matplotlib plotting; docs at
            <a href={pyDocsUrl} class="text-accent hover:underline"
              >jolars.github.io/eunoia-py</a
            >.
          </p>
        </div>
        <div class="bg-surface border border-line rounded p-4 space-y-2">
          <div class="flex items-center justify-between">
            <span class="font-medium inline-flex items-center gap-2"
              ><IconJulia class="w-4 h-4" /> Julia</span
            >
            <a href={juliaRepoUrl} class="text-xs text-accent hover:underline"
              >GitHub</a
            >
          </div>
          <pre
            class="bg-inset border border-line rounded p-2 font-mono text-xs overflow-x-auto">Pkg.add("Eunoia")</pre>
          <p class="text-xs text-muted">
            Bindings to the Rust core with Makie plotting; docs at
            <a href={juliaDocsUrl} class="text-accent hover:underline"
              >jolars.github.io/Eunoia.jl</a
            >.
          </p>
        </div>
      </div>
    ...
    <section class="space-y-4">
      ...
      >
        <li>
          Accurate and fast: uses exact n-way intersection geometry, analytical
          gradients, and efficient optimizers.
        </li>
        ...
        <li>
          Multiple loss functions to prioritize different aspects of fit
          quality.
        </li>
        <li>
          Customizable optimizers (CMA-ES, Levendberg-Marquardt, L-BFGS,
          Nelder-Mead).
        </li>
    ...
    <section class="space-y-3">
      ...
      <p
        class="text-sm bg-surface border border-line rounded p-4 leading-relaxed text-ink"
      >
        Larsson, J., &amp; Gustafsson, P. (2018). A case study in fitting
        area-proportional Euler diagrams with ellipses using eulerr. In
        Proceedings of International Workshop on Set Visualization and Reasoning
        2018.
        <a
          href="https://ceur-ws.org/Vol-2116/paper7.pdf"
          class="text-accent hover:underline"
          >https://ceur-ws.org/Vol-2116/paper7.pdf</a
        >
      </p>
      <p class="text-sm text-muted">
        Other formats (BibTeX, BibLaTeX, CSL, Vancouver, Chicago) are available
        on the
        <a href="/cite/" class="text-accent hover:underline">citation page</a>.
      </p>
    </section>

    <footer
      class="border-t border-line pt-6 text-xs text-muted flex flex-wrap items-center gap-x-4 gap-y-1"
    >
      <span
        >© 2025–{new Date().getFullYear()}
        <a
          href={johanUrl}
          class="text-accent inline-flex items-center hover:underline"
          >Johan Larsson</a
        ></span
      >
      <a href={repoUrl} class="hover:text-ink inline-flex items-center gap-1.5"
        ><IconGithub class="w-3.5 h-3.5" /> GitHub</a
      >
      <a
        href={cratesUrl}
        class="hover:text-ink inline-flex items-center gap-1.5"
        ><IconRust class="w-3.5 h-3.5" /> crates.io</a
      >
      <a
        href={docsRsUrl}
        class="hover:text-ink inline-flex items-center gap-1.5"
        ><IconBook class="w-3.5 h-3.5" /> docs.rs</a
      >
