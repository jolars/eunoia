      ...
      >
        ...
        {#each regions as region}
          {#each region.pieces as piece}
            <path d={piecePath(piece)} fill={regionFill(region.combination)} fill-opacity="0.75" />
          {/each}
        {/each}
        {#each regions as region}
          {#each region.pieces as piece}
        ...
        {#each regions as region}
          {#if region.combination !== ""}
            ...
            {@const isExterior =
              p?.kind === "exteriorRaycast" || p?.kind === "exteriorForceDirected"}
            {#if isExterior && p?.tether}
              <path
                d={leaderPath(p.tether, p.leaderEnd ?? anchor, p.leaderWaypoints)}
                fill="none"
                stroke="currentColor"
                stroke-opacity="0.45"
                stroke-width="0.4"
      ...
      {#each SLIDERS as s}
        <label class="block">
          ...
          <input
            ...
            value={sizes[s.key]}
            oninput={(e) =>
              (sizes[s.key] = Number((e.currentTarget as HTMLInputElement).value))}
