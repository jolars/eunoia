<script lang="ts">
import type { TrajectoryFrame, TrajectoryShape } from "@jolars/eunoia/trajectory";
...
type DisplayShape =
  ...
  | { kind: "ellipse"; label: string; x: number; y: number; a: number; b: number; phi: number };
...
const displayShapes: DisplayShape[] = $derived.by(() => {
  ...
  return a.map((sa: TrajectoryShape, idx: number): DisplayShape => {
    ...
    const eb = ("a" in sb ? sb : sa) as Extract<TrajectoryShape, { a: number }>;
    return {
      kind: "ellipse",
      label: sa.label,
      x: lerp(ea.x, eb.x, f),
    ...
    <div class="ml-1 inline-flex overflow-hidden rounded border border-line">
      <button
        ...
        class="px-3 py-1 disabled:cursor-not-allowed {shape === 'circle'
          ...
          : 'bg-inset text-ink hover:bg-accent-soft'}"
      >Circles</button>
      <button
        type="button"
        onclick={() => setShape("ellipse")}
        disabled={playing}
        class="border-l border-line px-3 py-1 disabled:cursor-not-allowed {shape === 'ellipse'
          ? 'bg-accent text-white'
          : 'bg-inset text-ink hover:bg-accent-soft'}"
      >Ellipses</button>
    </div>
