...
## 1.0.0 API stability audit

A full public-API audit ahead of a 1.0.0 release. Goal: get the surface right
*before* 1.0, because after it every breaking change is a major bump. Scope
covers all four artifacts --- the core crate (`crates/eunoia/`), the wasm
surface (`crates/eunoia-wasm/src/lib.rs`), the high-level TS wrapper
(`ts/index.ts`, `ts/svg.ts`), and the npm export map. Surfaced 2026-06-10.

Items are split into **P0** (irreversible after 1.0 --- must decide before the
release), **P1** (strongly recommended; cross-layer consistency), and **P2**
(polish / documentation, not blocking).
